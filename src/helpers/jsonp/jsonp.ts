...
export async function fetchTestCaseIndexFiles(indexFiles: TestCaseIndexFiles): Promise<TestCaseFiles> {
	const filesList = await Promise.all(
		entries(indexFiles)
			.map(([type, link]) =>
				fetchJsonp(link as string, jsonpHandlerNames.index[type])
					.then(filesInfo => ({ [type]: filesInfo as TestCaseFiles }))),
	);

	return filesList.reduce((files, currFile) => ({ ...files, ...currFile }), {}) as TestCaseFiles;
}
