...
import {
	jsonpHandlerNames, watchLiveTestCase, FileWatchCallback, watchReport,
} from '../jsonp/jsonp';
...
const MESSAGE_JSONP_PATH = jsonpHandlerNames.files.message;
const LOG_JSON_PATH = jsonpHandlerNames.files.logentry;
...
export default class LiveUpdateService {
	...
	public stopWatchingTestcase() {
		if (this.testCaseWatchIntervalId) {
	...
	private async fetchFullUpdateData(files: TestCaseFiles | null, testCaseOrder: number) {
		...
		await Promise.all(
			getObjectKeys(filesToLoad)
				.filter(fileType => Number(filesToLoad[fileType]?.count) > 0)
				.map(async fileType => {
			...
			getObjectKeys(filesToLoad)
				...
				.map(async fileType => {
					...
					for (const [filePath, index] of Object.entries(filesToLoad[fileType]!.dataFiles)) {
						if (filesToLoad[fileType]!.dataFiles[filePath] === prevFiles[fileType]!.dataFiles[filePath]) {
	...
	private async fetchFileUpdate(
		filePath: string, jsonpHandlerName: string, index: number, prevIndex: number, testCaseOrder: number,
	) {
