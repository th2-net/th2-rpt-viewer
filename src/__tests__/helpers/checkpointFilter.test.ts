...
describe('[Helpers] checkpointFilter', () => {
	test('getCheckpointActions(): no checkpoints list', () => {
		...
		const secondAction = createAction(2, [], 'test2');
	...
	test('getCheckpointActions(): flat checkpoint list', () => {
		...
		const secondCheckpoint = createAction(2, [], '2_GetCheckPoint', [{ name: 'Checkpoint', subParameters: [] }]);
		const thirdCheckpoint = createAction(3, [], '3_GetCheckPoint', [{ name: 'Checkpoint', subParameters: [] }]);
		const actions = [firstAction, secondCheckpoint, thirdCheckpoint];
	...
	test('getCheckpointActions(): checkpoint in tree', () => {
		const checkpoint = createAction(0, [], 'GetCheckPoint', [{ name: 'Checkpoint', subParameters: [] }]);
		const secondAction = createAction(2, [checkpoint], 'test');
		const firstAction = createAction(1, [secondAction], 'test');
