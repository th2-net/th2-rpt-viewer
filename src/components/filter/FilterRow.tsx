...
const PATH_OPTIONS = [FilterPath.ALL, FilterPath.STATUS, FilterPath.SERVICE];

const AUTOCOMPLETE_MAP = new Map<FilterPath, string[]>([
	[
		FilterPath.STATUS,
		[StatusType.PASSED, StatusType.FAILED, StatusType.CONDITIONALLY_PASSED, StatusType.CONDITIONALLY_FAILED],
	],
]);
...
export default function FilterRow(props: Props) {
	...
	const getAutocomplete = () => {
		...
		if (values && AUTOCOMPLETE_MAP.has(path)) {
	...
	const bubbleIsValid = (value: string) => {
		if (path && AUTOCOMPLETE_MAP.has(path)) {
				...
				path != null ? (
					<React.Fragment>
						...
						<Select
							...
							selected={path}
							onChange={(nextPath: string) =>
								submitChange({ ...tempBlock, path: nextPath as FilterPath })}
						/>
					</React.Fragment>
				) : null
			}
