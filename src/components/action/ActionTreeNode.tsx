...
interface Props {
    ...
    expandPath: Tree<ActionExpandStatus> | null;
    filter: {
        results: string[];
        isActive: boolean;
...
export default function ActionTreeNode(props: Props) {
	...
	case ActionNodeType.ACTION: {
		...
		return (
			<ActionCard
				...
				isRoot={isRoot}
				isExpanded={expandPath ? expandPath.value.isExpanded : false}
	...
	case ActionNodeType.VERIFICATION: {
		...
		const isSelected = verification.messageId === selectedVerificationId;
