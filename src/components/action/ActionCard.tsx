...
export function ActionCard(props: CardProps) {
	...
	const {
		id, name, status, parameters,
	} = action;

	const isEmptyParameters = parameters === null;
	const isSingleEmptyMessage = !isEmptyParameters
			&& parameters.length === 1
			&& parameters[0].name === 'Message'
			&& parameters[0].value === ''
			&& parameters[0].subParameters!.length === 0;
	const showParams = !isEmptyParameters && !isSingleEmptyMessage;
	const isDisabled = !showParams
			&& !action.isTruncated
	...
	return (
		<div className={rootClassName}
			...
			<ExpandablePanel
				...
				onExpand={isPanelExpanded => onRootExpand(isPanelExpanded)}>
				{
					toggleExpand => (
						<ActionCardHeader
							action={action}
				...
				<div className="ac-body">
					...
					}
					{
						...
						&& <div className="ac-body__truncated-warning">
							{action.verificationCount - action.subNodes.filter(
								node => isVerification(node),
